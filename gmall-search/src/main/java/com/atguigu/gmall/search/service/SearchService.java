...
public class SearchService {
    ...
    public SearchResponseVo search(SearchParamVo paramVo) {
        try {
            ...
            SearchResponse response = restHighLevelClient.search(new SearchRequest(new String[]{"goods"},sourceBuilder), RequestOptions.DEFAULT);
        ...
        if(!CollectionUtils.isEmpty(idAggBuckets)){
            ...
            List<SearchResponseAttrVo> attrVos = idAggBuckets.stream().map(bucket -> {
                ...
                if(!CollectionUtils.isEmpty(attrNameBuckets)){
                    responseAttrVo.setAttrName(attrNameBuckets.get(0).getKeyAsString());
                }
