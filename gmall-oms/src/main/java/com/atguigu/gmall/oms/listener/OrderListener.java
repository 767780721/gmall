...
public class OrderListener {
    ...
    public void failure(String orderToken, Channel channel, Message message) throws IOException {
        ...
        if(StringUtils.isNotBlank(orderToken)){
            orderMapper.updateStatus(orderToken,5,0);
        }
    ...
    public void close(String orderToken, Channel channel, Message message) throws IOException {
        ...
        if(StringUtils.isNotBlank(orderToken)){
            if(orderMapper.updateStatus(orderToken,4,0) == 1){
    ...
    public void success(String orderToken, Channel channel, Message message) throws IOException{
        if(StringUtils.isNotBlank(orderToken) && orderMapper.updateStatus(orderToken,1,0) == 1){
            //发送消息给wms  减库存
            rabbitTemplate.convertAndSend("ORDER_EXCHANGE","stock.minus",orderToken);
            //发送消息给usm 加积分
            //查询订单获取用户id
            Map<String,Object> map = new HashMap<>();
            rabbitTemplate.convertAndSend("ORDER_EXCHANGE","user.bounds",map);
        }
        channel.basicAck(message.getMessageProperties().getDeliveryTag(),false);
    }
