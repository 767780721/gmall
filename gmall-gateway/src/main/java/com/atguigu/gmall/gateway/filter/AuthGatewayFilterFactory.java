    ...
    public GatewayFilter apply(PathConfig config) {
        return new GatewayFilter() {
            ...
            public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
                System.out.println("这是局部过滤器,只过滤配置了该过滤器的服务。 " + config);
                //webflux提供的request对象 等价于HttpServletRequest对象
                ServerHttpRequest request = exchange.getRequest();
                //等价于 HttpServletResponse对象
                ServerHttpResponse response = exchange.getResponse();

                //1.判断当前路径在不在拦截名单中，不在直接放行
                List<String> pathes = config.pathes;
                //webflux中的uri === url 为全路径  获取当前请求的路径
                String curPath = request.getURI().getPath();
                //如果拦截名单为空 或者 当前请求的路径没有包含任何一个白名单中的路径  放行
                if(CollectionUtils.isEmpty(pathes) || !pathes.stream().anyMatch(path -> curPath.startsWith(path))){
                    return chain.filter(exchange);
                }

                //2.获取token信息： 异步通过头信息来去传递token  同步方式只能通过携带cookie的方式来传递token
                ...
                try {
                    //4.解析token：解析出现异常，重定向到登录页面
                    Map<String, Object> map = JwtUtils.getInfoFromToken(token, properties.getPublicKey());

                    //5.防盗用，验证jwt载荷中的ip，是否和当前请求的ip一致，则重定向到登录页面
                    String curIp = IpUtil.getIpAddressAtGateway(request); //当前用户的ip
